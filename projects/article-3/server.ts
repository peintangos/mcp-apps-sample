...
const UI_RESOURCE_URI = "ui://claude-second-opinion/mcp-app.html";
...
Workflow: first answer the user's question yourself concisely in the chat, then immediately invoke this tool in the same turn. The iframe will render Claude's independent view underneath.`,
  ...
  registerAppResource(
    ...
    {
      description:
        "Article 3 UI resource. Renders the ask_claude placeholder in spec-001 and the side-by-side comparison view from spec-003 onward.",
    },
    async () => ({
      contents: [
        {
...
app.listen(PORT, () => {
  console.log(
    `[article-3] MCP server listening on http://localhost:${PORT}/mcp`,
  );
});
