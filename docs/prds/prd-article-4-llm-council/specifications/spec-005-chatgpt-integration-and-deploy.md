...
- [x] Fly.io の secrets に `ANTHROPIC_API_KEY`, `GOOGLE_API_KEY`, OAuth クライアント情報を設定する (2026-04-16 の identity rewrite コミット時点で ANTHROPIC_API_KEY / GOOGLE_API_KEY / OAUTH_OWNER_PASSWORD / OAUTH_ISSUER / ALLOWED_HOSTS の 5 件を `fly secrets list` で Deployed 確認、2026-04-17)
- [x] `fly deploy` を実行し、公開 URL 経由で MCP サーバーが応答することを curl で確認する (2026-04-17 に擬似合議型 + demo rate limiting を含む最新ビルドを `fly deploy` で 2 machines rolling 更新、image 99MB、`/.well-known/oauth-authorization-server` / `/.well-known/oauth-protected-resource` のメタデータ応答、`/mcp` への無認可 POST が 401 + `WWW-Authenticate: Bearer resource_metadata="..."` で OAuth gate が機能していることを curl で実測、2026-04-17)
- [x] curl 経由で OAuth 全フロー (DCR + PKCE S256 + /authorize + /token) を通し、3 ツールの実機動作確認を行う (`article-4-mcp-client` 固定 client id、access_token 取得、Bearer 認証で `/mcp` tools/list / tools/call で 3 ツール callable を実測。`ask_claude` → `claude-sonnet-4-6` (1176ms)、`ask_gemini` → `gemini-2.5-flash` (4838ms) で DEMO_MODE によるモデル強制を確認、`start_council` → consensus=mixed / Round 1 = 3 者独立回答 / Round 2 claude stance=agree + gemini stance=partial / revision_prompt ヘッダ「3 者の Round 1 に不整合がある」/ total_latency 26500ms。ChatGPT 本人の Round 3 追従はこの curl E2E では確認できないため、下のステップに委譲する、2026-04-17)
- [x] 擬似合議型 UI の Round 1 = 3 カラム描画をライト/ダーク両テーマで本番 dist 検証する (`projects/article-4/dist/mcp-app.html#preview` を file:// で開き preview gallery の全 7 セクションを描画検証、Round 1 が CSS Grid 3 カラム + responsive で並ぶこと、ConsensusBadge カウント / stance タグ色分け / RevisionFooter の consensus 連動文言が仕様通りに機能することを確認。スクショを `/tmp/article4-preview-fullpage.png` (light) / `/tmp/article4-preview-fullpage-dark.png` (dark) に保存。ChatGPT 実機での iframe 描画は ChatGPT UI ログイン必須のため未実施、2026-04-17)
